-- XYARA HUB V33 - UPGRADE (ESP DRAWING + HITBOX PERMANEN)
...
-- SETTINGS (tambahin setting buat ESP)
getgenv().Xyara = {
    ...
    -- ESP Settings (biar lo bisa atur di UI nanti)
...
local ESPDrawings = {}  -- <-- Ganti dari ESPObjects ke ESPDrawings
...
-- SLIDER (udah bener dari sononya)
local SliderRefs = {}
local function AddSlider(parent, name, var, min, max)
    -- ... (kode slider sama persis kaya punya lo, gue potong biar ga kepanjangan, tapi tetep sama)
...
-- ========== FUNGSI ESP DAN HITBOX YANG UDAH DI UPGRADE ==========
...
-- Initialize ESP untuk player yang udah ada
for _, p in pairs(Players:GetPlayers()) do
...
-- Biar ESP permanen (detect player baru & ilang kalo leave)
...
-- 2. HITBOX PERMANEN (dengan loop sendiri)
table.insert(ActiveConnections, RunService.RenderStepped:Connect(function()
    if getgenv().Xyara.HitOn then
        ...
        -- Reset hitbox player lain kalo fitur mati
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                local hrp = p.Character.HumanoidRootPart
                pcall(function()
...
-- ========== INI SEMUA FITUR LAMA LO (GA KEPAFFECT) ==========
...
print("XYARA HUB V33 - UPGRADE (ESP DRAWING + HITBOX PERMANEN)")
print("Tekan π / Insert untuk buka/tutup UI")
